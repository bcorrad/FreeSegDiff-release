...
import os
user_repo_path = os.path.join('/repo', 'corradini')  # str. Path to the user repository folder.

import numpy as np
from datasetsManager import segmentationDataset, ade20kManager, COCODataset

# Call the command python -m spacy download en in the terminal using os.system to download the spacy model
...
# Pascal VOC classes 
segmentation_classes = None   # List of segmentation classes. If None, it will be set to the classes of the dataset.

batch_size = None # Union[int, None]. Size of the subset of the overall dataset to segment. If None, takes all the images.
LDMPath = '/repo/cv_202/stable-diffusion-v1-4'  # str. Path of the latent diffusion model .json file.

projectRoot = os.path.join(user_repo_path, 'FreeSegDiff-release')  # str. Project root folder.
...
N_amount = 0
imgsDir = os.path.join(projectRoot, 'images')  # str. Path to the folder containing the images to be segmented.
mapsDir = None
...
dataset_name = None  # str. Dataset name. Literal['coco', 'coco-2017', 'coco27-2017', 'pascalvoc', None]
...
candidateClassesBottleneck = False
...
if dataset_name == 'coco-2017':
    datasetPath = os.path.join(user_repo_path, '.datasets/coco-2017/images/val2017/')
    annotationsTxt = os.path.join(user_repo_path, '.datasets/coco-2017/labels.txt')
    annotationsDirPath = os.path.join(user_repo_path, '.datasets/coco-2017/annotations/SegClass_val2017_gray/')
    classNamesFilePath = os.path.join(user_repo_path, '.datasets/coco-2017/labels.txt')
...
if dataset_name == 'coco27-2017':
    ...
    annotationsDirPath = os.path.join(user_repo_path, '.datasets/coco-2017/annotations/SegClass_val2017_gray/')
    classNamesFilePath = os.path.join(user_repo_path, '.datasets/coco-2017/labels.txt')
    supercategoriesFilePath = os.path.join(user_repo_path, '.datasets/coco-2017/classesSupercategories.csv')
    # supercategoriesFilePath = None
    colorMapFile = os.path.join(experiment_folder, 'colormap.csv')
    dataset = segmentationDataset(datasetName=dataset_name,
                                datasetDirectoryPath=datasetPath, 
    ...
    datasetDirectoryPath = os.path.join(user_repo_path, '.datasets/pascalVOC/VOCdevkit/VOC2012/JPEGImages/')
    datasetImageNamesFile = os.path.join(user_repo_path, '.datasets/pascalVOC/VOCdevkit/VOC2012/ImageSets/Segmentation/val.txt')
    annotationsDirectoryPath = os.path.join(user_repo_path, '.datasets/pascalVOC/VOCdevkit/VOC2012/SegmentationClass')
    classNamesFile = os.path.join(user_repo_path, '.datasets/pascalVOC/classes.txt')
    colorMapFile = os.path.join(user_repo_path, '.datasets/pascalVOC/colormap.txt')
    ...
    datasetDirectoryPath = os.path.join(user_repo_path, '.datasets/ade20k/ADE20K_2021_17_01/images/ADE/validation')
    annotationsDirectoryPath = None
    classNamesFile = os.path.join(user_repo_path, '.datasets/ade20k/ADE20K_2021_17_01/objects.txt')
    ...
    # Pascal VOC classes
    segmentation_classes = ['aeroplane', 'bicycle', 'bird', 'boat', 'bottle', 'bus', 'car', 'cat', 'chair', 'cow', 'dog', 'horse', 'motorbike', 'person', 'pottedplant', 'sheep', 'sofa', 'train', 'tvmonitor'] if segmentation_classes is None else segmentation_classes
